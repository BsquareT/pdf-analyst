...
//  Set AI_PROVIDER in Vercel environment variables:
//    "gemini"  → Google Gemini Flash (FREE tier: 1500 req/day)
//    "claude"  → Anthropic Claude (paid, higher quality)
//    "haiku"   → Anthropic Claude Haiku (paid but ~20x cheaper than Sonnet)
//
//  Start with "gemini" (free), switch to "haiku" or "claude" when ready.
// ══════════════════════════════════════════════════════════════════
...
async function callAI(prompt) {
  switch (PROVIDER) {
    ...
    case 'haiku':
      return callClaude(prompt, 'claude-haiku-4-5-20251001');
    case 'claude':
    default:
...
export default async function handler(req, res) {
  ...
  const prompt = [
    ...
    'If excerpts do not contain enough info for a section, write "Not found in the provided excerpts."',
  ].join('\n');
  ...
  if (!isUnlimited) {
    ...
    await supabaseAdmin.from('profiles')
      .update({ questions_remaining: newRemaining }).eq('id', user.id);
  }
